...
* **Your Answer:** It deletes all documents in the collection and then writes the canned records back to the collection.
...
* **Your Answer:**  Your account info is written to the database (encrypted I hope).
...
* **Your Answer:**  It asks you for your username and password, then retrieves the password from the database that matches that username, and tests equality.
...
* **Your Answer:**  Your account document needs to have a list of roles you have.
...
* **Your Answer:**  The password is stored unencrypted is probably the biggest problem.

* **Question:** What would happen if three different users tried to sign up with the same username? How can we prevent that?

* **Your Answer:**  Currently it would allow it.  There are multiple methods to prevent this.  It is possible to enforce uniqueness either in the database or in the Model, or do a get before post (could have concurrency issues)

* **Question:** Why are we making our route `POST /api/signup` as opposed to `POST /api/users`?

* **Your Answer:**  Signup gives more of an indication that it is something that a user does on their own behalf, rather than an administrative function.
...
* **Your Answer:**  Header:  Type is always JWT, alg is one of several encryption algorithms.
Payload is a list of "claims."  Some of these are common, such as expiration; others are internal to the application, like roles and permissions of the user.
...
* **Your answer:**  Roles and permissions of the user, the username.
...
* **Your Answer:**  Any secrets or passwords.
...
* **Your Answer:**  The payload is what we send back that is unique to our application.  The passcode is for encryption.  Options is some subset of an enumerated set of options, such as expiresIn.
...
* **Your Answer:**  Authentication is what allows the user in the front door.  Authorization is what roles or permissions the user has.
...
* **Your Answer:**  If no header is sent then we get a 401.
...
* **Your Answer:**  Everything about the profile is returned, other than the password
